...
class yolov3(object):
    ...
    def __init__(self, num_classes, anchors,
        ...
        self._ANCHORS = anchors     #锚框大小
        self._BATCH_NORM_DECAY = batch_norm_decay
        self._LEAKY_RELU = leaky_relu
        self._NUM_CLASSES = num_classes     #类别数量
        self.feature_maps = [] # [[None, 13, 13, 255], [None, 26, 26, 255], [None, 52, 52, 255]]       #三种特征图大小
    ...
    def _broadcast_iou(self, true_box_xy, true_box_wh, pred_box_xy, pred_box_wh):
        ...
        # shape: [N, 13, 13, 3, 1, 2]
        pred_box_xy = tf.expand_dims(pred_box_xy, -2)   #X1_p,Y1_p
        pred_box_wh = tf.expand_dims(pred_box_wh, -2)   #X2_p-X1_p,Y2_p-Y1_p

        # shape: [1, V, 2]
        true_box_xy = tf.expand_dims(true_box_xy, 0)    #X1_b,Y1_b
        true_box_wh = tf.expand_dims(true_box_wh, 0)    #X2_b-X1_b,Y2_b-Y1_b
