...
def iou(box, clusters):
    ...
    intersection = x * y        #I 相交的面积
    box_area = box[0] * box[1]  #A_b bounding box 实际值
    cluster_area = clusters[:, 0] * clusters[:, 1]  #A_p 预测值

    iou_ = intersection / (box_area + cluster_area - intersection)  #交并比

    x1_c=np.maximum(clusters[:, 0], box[0])
    #giou=
